...
fn load_data(buffer: &[u8], dtype: &str, offset: usize, dimensions: &[u32]) -> ArrayD<f64> {
    ...
    let data: Vec<f64> = match dtype {
        ...
        "i16" => {
            ...
            (0..total_elements).map(|_| cursor.read_i16::<BigEndian>().unwrap() as f64).collect()
        }
        "i32" => {
            let mut cursor = Cursor::new(data_slice);
            (0..total_elements).map(|_| cursor.read_i32::<BigEndian>().unwrap() as f64).collect()
        }
        "f32" => {
            let mut cursor = Cursor::new(data_slice);
            (0..total_elements).map(|_| cursor.read_f32::<BigEndian>().unwrap() as f64).collect()
        }
        "f64" => {
            let mut cursor = Cursor::new(data_slice);
            (0..total_elements).map(|_| cursor.read_f64::<BigEndian>().unwrap()).collect()
        }
...
fn reshape_data(data: ArrayD<f64>, dimensions: &[u32]) -> ArrayD<f64> {
    if dimensions.len() > 1 {
        ...
        data.into_shape(shape).expect("重塑数组失败").to_owned().into_dyn()
    } else {
        data
    }
}
...
mod tests {
    ...
    let image = image_flat.into_shape((28, 28)).expect("无法将图像重塑为 28x28");
