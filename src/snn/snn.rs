    ...
    > SNN<N, NET_INPUT_DIM, NET_OUTPUT_DIM>
{
    ...
    pub fn process<const SPIKES_DURATION: usize>(
        &mut self,
        spikes: &[[u8; SPIKES_DURATION]; NET_INPUT_DIM],
        ...
        for (n, neuron_spikes) in spikes.into_iter().enumerate() {
            ...
            for t in 0..spikes_duration.unwrap() {
                ...
                if temp_spike != 0 && temp_spike != 1 {
                    panic!(
                        "错误：输入 spike 必须为 0 或 1，对于神经元 {} 在 t={}",
                        n, t
                    );
                }
    ...
    */
    fn encode_spikes<const SPIKES_DURATION: usize>(
        spikes: &[[u8; SPIKES_DURATION]; NET_INPUT_DIM],
    ) -> Vec<SpikeEvent> {
    ...
    /**
        ...
        每行对应一个输出神经元
    */
    fn decode_spikes<const SPIKES_DURATION: usize>(
