    ...
    WeightsBuilder<N, INPUT_DIM, NET_INPUT_DIM>
{
    ...
    */
    pub fn weights<const NUM_NEURONS: usize>(
        mut self,
        weights: [[f64; INPUT_DIM]; NUM_NEURONS],
    ...
    NeuronsBuilder<N, NUM_NEURONS, NET_INPUT_DIM>
{
    ...
    */
    ...
    /**
        ...
        - 所有神经元具有相同的参数
    */
    pub fn neurons_with_same_parameters(
        mut self,
        neuron: N,
        num_neurons: usize,
    ) -> IntraWeightsBuilder<N, NUM_NEURONS, NET_INPUT_DIM> {
...
/* * 内部权重 * */
#[derive(Debug, Clone)]
pub struct IntraWeightsBuilder<
    N: Neuron + Clone + Send + 'static,
    const NUM_NEURONS: usize,
    const NET_INPUT_DIM: usize,
> {
    ...
    IntraWeightsBuilder<N, NUM_NEURONS, NET_INPUT_DIM>
{
    ...
    */
    pub fn intra_weights(
        mut self,
        intra_weights: [[f64; NUM_NEURONS]; NUM_NEURONS],
        ...
        for neuron_intra_weights in &intra_weights {
            neuron_intra_weights.iter().for_each(|w| {
                if w > &0f64 {
                    panic!("内部权重必须是负数");
                }
            });
