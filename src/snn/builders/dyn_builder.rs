...
/**
    ...
    它允许通过传递所有网络参数（神经元、额外权重、内部权重等）到一个函数中来一次性配置网络。
...
impl<N: Neuron + Clone> DynSnnBuilder<N> {
    ...
    /**
        它会进行所有与网络的内部权重相关的检查。
        - 它检查神经元的数量是否等于内部权重矩阵的行数
        - 它检查神经元的数量是否等于内部权重矩阵的列数
        - 它检查内部权重的值是否都是负数，并且在[-1, 0]范围内
    */
    ...
    /**
        它会进行所有与网络的额外权重相关的检查。
        - 它检查神经元的数量是否等于额外权重矩阵的行数
        - 它检查神经元的数量是否等于额外权重矩阵的列数
        - 它检查额外权重矩阵的列数是否等于前一层的神经元数量
        - 它检查额外权重的值是否都是正数，并且在[0, 1]范围内
    */
